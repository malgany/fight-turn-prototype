...
begin
  ...
  then
    new.turn_deadline_at = greatest(new.turn_deadline_at, now() + interval '10 seconds');
  end if;
...
set turn_deadline_at = now() + interval '10 seconds'
where status = 'active'
  and turn_deadline_at < now() + interval '4 seconds';
