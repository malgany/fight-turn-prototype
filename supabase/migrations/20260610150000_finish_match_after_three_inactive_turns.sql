...
drop trigger if exists finish_match_after_three_inactive_turns on public.matches;

create trigger finish_match_after_three_inactive_turns
before update of current_turn, last_turn, status on public.matches
for each row
execute function public.finish_match_after_three_inactive_turns();
